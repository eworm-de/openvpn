           ...
           sitnl_parse_reply_cb cb, void *arg_cb)
{
    ...
    while (1)
    {
        ...
        {
            msg(M_WARN, "%s: sender address length: %u (expected %lu)",
...
int
net_route_v4_best_gw(const in_addr_t *dst, int prefixlen, in_addr_t *best_gw,
                       char *best_iface)
{
...
sitnl_route_set(int cmd, uint32_t flags, int ifindex, sa_family_t af_family,
                ...
                enum rt_scope_t scope, int type)
{
    struct sitnl_route_req req;
    int ret = -1, size;
...
static int
sitnl_route_add(const char *iface, sa_family_t af_family,
                const void *dst, int prefixlen, const void *gw, uint32_t table,
                int metric)
{
    int ifindex = 0;
                ...
                int metric)
{
    ...
    return sitnl_route_set(RTM_DELROUTE, 0, ifindex, af_family, dst, prefixlen,
                           gw, table, metric, RT_SCOPE_NOWHERE, 0);
}
